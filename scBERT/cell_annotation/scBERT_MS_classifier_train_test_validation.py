...
X = np.load("outputs/ms_X_cls.npy")
y = np.load("outputs/ms_y_cls.npy", allow_pickle=True)
...
print("Predicted label distribution:", np.unique(predicted_labels, return_counts=True))  
...
plt.figure(figsize=(10, 6))  # 从 (8, 6) 改成 (8, 8) 或者 (10, 7)
sns.scatterplot(
    x=X_umap[:, 0],
    y=X_umap[:, 1],
    hue=y_all,
    palette="tab20",  # 18种颜色可换更丰富的tab20
    s=8,
    linewidth=0,
    legend='full'
)
plt.axis("equal")  # 保持x/y缩放一致

plt.title("UMAP of CLS Embeddings (All MS Data)")
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.savefig("figures_0420/umap_MS.svg", format='svg')
plt.close()
print("UMAP embedding figure saved to: figures_0420/umap_MS.svg")